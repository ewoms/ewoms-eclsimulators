...
{
    if (m_comm.rank() != 0)
        EWOMS_THROW(std::runtime_error, "reset_actnum should only be called on root process.");
...
const FieldPropsManager& ParallelEclipseState::fieldProps() const
{
    if (!m_parProps && Dune::MPIHelper::getCollectiveCommunication().rank() != 0)
        EWOMS_THROW(std::runtime_error, "Attempt to access field properties on no-root process before switch to parallel properties");
...
const FieldPropsManager& ParallelEclipseState::globalFieldProps() const
{
    if (Dune::MPIHelper::getCollectiveCommunication().rank() != 0)
        EWOMS_THROW(std::runtime_error, "Attempt to access global field properties on non-root process");
...
{
    if (Dune::MPIHelper::getCollectiveCommunication().rank() != 0)
        EWOMS_THROW(std::runtime_error, "Attempt to access eclipse grid on non-root process");
