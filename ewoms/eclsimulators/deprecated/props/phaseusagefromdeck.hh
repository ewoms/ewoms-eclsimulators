...
*/
#ifndef EWOMS_PHASEUSAGEFROMDECK_HH
#define EWOMS_PHASEUSAGEFROMDECK_HH
...
namespace Ewoms
{
    ...
    inline PhaseUsage phaseUsage(const Phases& phases) {
        ...
        if (!pu.phase_used[BlackoilPhases::Liquid] && !(pu.num_phases == 1)) {
            EWOMS_THROW(std::runtime_error, "Cannot handle cases with no OIL, i.e. water-gas systems.");
        }
        ...
        if (pu.has_polymermw) {
            if (!pu.has_polymer) {
                EWOMS_THROW(std::runtime_error, "pu.has_polymermw is true while pu.has_polymer is false");
            }
    ...
    inline PhaseUsage phaseUsageFromDeck(const Ewoms::EclipseState& eclipseState)
    {
    ...
    /// to determine active phases.
    inline PhaseUsage phaseUsageFromDeck(const Ewoms::Deck& deck)
    {
...
#endif // EWOMS_PHASEUSAGEFROMDECK_HH
