...
namespace Ewoms
{
    ...
    {
        if( decayrate_  > 1.0 ) {
            EWOMS_THROW(std::runtime_error,"SimpleIterationCountTimeStepControl: decay should be <= 1 " << decayrate_ );
        }
        if( growthrate_ < 1.0 ) {
            EWOMS_THROW(std::runtime_error,"SimpleIterationCountTimeStepControl: growth should be >= 1 " << growthrate_ );
        }
    }
    ...
    HardcodedTimeStepControl( const std::string& filename)
    {
        ...
        if (!infile.is_open()) {
            EWOMS_THROW(std::runtime_error,"Incorrect or no filename is provided to the hardcodedTimeStep. Use timestep.control.filename=your_file_name");
        }
