...
#include <ewoms/eclsimulators/timestepping/adaptivesimulatortimer.hh>
...
NEW_TYPE_TAG(EFlowTimeSteppingParameters);
...
SET_SCALAR_PROP(EFlowTimeSteppingParameters, SolverMaxGrowth, 3.0);
SET_SCALAR_PROP(EFlowTimeSteppingParameters, SolverMaxTimeStepInDays, 365.0);
SET_SCALAR_PROP(EFlowTimeSteppingParameters, SolverMinTimeStep, 0.0);
SET_INT_PROP(EFlowTimeSteppingParameters, SolverMaxRestarts, 10);
SET_INT_PROP(EFlowTimeSteppingParameters, SolverVerbosity, 1);
SET_INT_PROP(EFlowTimeSteppingParameters, TimeStepVerbosity, 1);
...
namespace Ewoms {
    ...
    class AdaptiveTimeStepper
    {
        ...
        class SolutionTimeErrorSolverWrapperEebos : public RelativeChangeInterface
        {
            const Solver& solver_;
        public:
            SolutionTimeErrorSolverWrapperEebos(const Solver& solver)
              : solver_(solver)
            {}
    ...
    public:
        ...
        AdaptiveTimeStepper(const bool terminalOutput = true)
            : timeStepControl_()
                             ...
                             const std::vector<int>* fipnum = nullptr)
        {
            ...
            while (!substepTimer.done()) {
                ...
                if (substepReport.converged) {
                    ...
                    SolutionTimeErrorSolverWrapperEebos<Solver> relativeChange(solver);
                    ...
                    if (restarts >= solverRestartMax_) {
                        ...
                        EWOMS_THROW_NOLOG(Ewoms::NumericalIssue, msg);
                    }
    ...
    protected:
        void init_()
        {
            ...
            if (control == "pid") {
                ...
                EWOMS_THROW(std::runtime_error,"Unsupported time step control selected "<< control);
