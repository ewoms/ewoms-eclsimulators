...
NEW_TYPE_TAG(EFlowNonLinearSolver);

NEW_PROP_TAG(Scalar);
NEW_PROP_TAG(NewtonMaxRelax);
NEW_PROP_TAG(EFlowNewtonMaxIterations);
NEW_PROP_TAG(EFlowNewtonMinIterations);
NEW_PROP_TAG(NewtonRelaxationType);

SET_SCALAR_PROP(EFlowNonLinearSolver, NewtonMaxRelax, 0.5);
SET_INT_PROP(EFlowNonLinearSolver, EFlowNewtonMaxIterations, 20);
SET_INT_PROP(EFlowNonLinearSolver, EFlowNewtonMinIterations, 1);
SET_STRING_PROP(EFlowNonLinearSolver, NewtonRelaxationType, "dampen");
    ...
    public:
        ...
        struct SolverParameters
        {
            ...
            SolverParameters()
            {
                ...
                maxIter_ = EWOMS_GET_PARAM(TypeTag, int, EFlowNewtonMaxIterations);
                minIter_ = EWOMS_GET_PARAM(TypeTag, int, EFlowNewtonMinIterations);
                ...
                if (relaxationTypeString == "dampen") {
                    ...
                    EWOMS_THROW(std::runtime_error, "Unknown Relaxtion Type " << relaxationTypeString);
                }
            }

            static void registerParameters()
            {
                EWOMS_REGISTER_PARAM(TypeTag, Scalar, NewtonMaxRelax, "The maximum relaxation factor of a Newton iteration used by eflow");
                EWOMS_REGISTER_PARAM(TypeTag, int, EFlowNewtonMaxIterations, "The maximum number of Newton iterations per time step used by eflow");
                EWOMS_REGISTER_PARAM(TypeTag, int, EFlowNewtonMinIterations, "The minimum number of Newton iterations per time step used by eflow");
                EWOMS_REGISTER_PARAM(TypeTag, std::string, NewtonRelaxationType, "The type of relaxation used by eflow's Newton method");
            }
        ...
        {
            if (!model_) {
                EWOMS_THROW(std::logic_error, "Must provide a non-null model argument for NonlinearSolver.");
            }
        }
        ...
        void stabilizeNonlinearUpdate(BVector& dx, BVector& dxOld, const double omega) const
        {
            ...
            default:
                EWOMS_THROW(std::runtime_error, "Can only handle Dampen and SOR relaxation type.");
            }
