...
*/
#ifndef EWOMS_PARALLELOVERLAPPINGILU0_HH
#define EWOMS_PARALLELOVERLAPPINGILU0_HH
...
template<class M, class X, class Y, class C>
struct SmootherTraits<Ewoms::ParallelOverlappingILU0<M,X,Y,C> >
{
    using Arguments = Ewoms::ParallelOverlappingILU0Args<typename M::field_type>;
};
                             ...
                             std::vector<typename M::block_type>* diagonal = nullptr)
    {
        ...
        for ( auto irow = A.begin(), iend = A.end(); irow != iend; ++irow)
        {
            ...
            if ( a_ik.index() != irow.index() )
                EWOMS_THROW(std::logic_error, "Matrix is missing diagonal for row " << irow.index());
...
public:
    ...
    */
    virtual void apply (Domain& v, const Range& d) override
    {
        ...
        if( iEnd != upper_.rows() )
        {
            EWOMS_THROW(std::logic_error,"ILU: number of lower and upper rows must be the same");
        }
    ...
    */
    ...
    virtual void update() override
    {
        ...
        if ( comm_ && comm_->communicator().size()<=0 )
        {
            if ( A_->N() > 0 )
            {
                EWOMS_THROW(std::logic_error, "Expected a matrix with zero rows for an invalid communicator.");
            }
            else
            {
