...
#include <ewoms/eclsimulators/linalg/bda/bdabridge.hh>
...
public:
  ...
  WellModelMatrixAdapter (const M& A,
                          ...
                          const boost::any& parallelInformation EWOMS_UNUSED_NOMPI = boost::any() )
    ...
    public:
              ...
              converged_(false)
        {
            ...
            if (use_gpu && !matrix_add_well_contributions) {
                EWOMS_THROW(std::logic_error,"Error cannot use GPU solver if command line parameter --matrix-add-well-contributions is false, because the GPU solver performs a standard bicgstab");
            }
            bdaBridge.reset(new BdaBridge(use_gpu, linear_solver_verbosity, maxit, tolerance));
#else
            const bool use_gpu = EWOMS_GET_PARAM(TypeTag, bool, UseGpu);
            if (use_gpu) {
                EWOMS_THROW(std::logic_error,"Error cannot use GPU solver since CUDA was not found during compilation");
            }
#endif
                                             ...
                                             Dune::InverseOperatorResult& result) const
        {
            ...
            if( parameters_.linear_solver_use_amg_ || parameters_.use_cpr_)
            {
                ...
                if (  parameters_.use_cpr_ )
                {
                    ...
                    using CouplingMetric = Ewoms::Amg::Element<pressureEqnIndex, pressureVarIndex>;
...
#endif
            {
                EWOMS_THROW(std::logic_error,"this method if for parallel solve only");
            }
    ...
    protected:
        ...
        Vector getStorageWeights() const
        {
            ...
            for (; elemIt != elemEndIt; ++elemIt) {
                ...
                MatrixBlockType block_transpose = Ewoms::transposeDenseMatrix(block);
