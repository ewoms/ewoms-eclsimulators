...
public:
    ...
    const std::vector<double>& updateOwnerMask(const T& container) const
    {
        if( ! indexSet_ )
        {
            EWOMS_THROW(std::runtime_error, "Trying to update owner mask without parallel information!");
        }
    ...
    /// The possible functors needed can be constructed with Ewoms::Reduction::makeGlobalMaxFunctor(),
    /// Ewoms::Reduction::makeLInfinityNormFunctor(),
    /// Ewoms::Reduction::makeGlobalMinFunctor(), and
    /// Ewoms::Reduction::makeGlobalSumFunctor().
