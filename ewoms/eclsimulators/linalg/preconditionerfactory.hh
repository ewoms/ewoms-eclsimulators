...
#ifndef EWOMS_PRECONDITIONERFACTORY_HEADER
...
private:
    ...
    static auto amgSmootherArgs(const boost::property_tree::ptree& prm,
                                Id<Ewoms::ParallelOverlappingILU0<Matrix, Vector, Vector, Comm>>)
    {
        using Smoother = Ewoms::ParallelOverlappingILU0<Matrix, Vector, Vector, Comm>;
        using SmootherArgs = typename Dune::Amg::SmootherTraits<Smoother>::Arguments;
        SmootherArgs smootherArgs;
        smootherArgs.iterations = prm.get<int>("iterations", 1);
    ...
    createParILU(const Operator& op, const boost::property_tree::ptree& prm, const Comm& comm, const int ilulevel)
    {
        ...
        if (ilulevel == 0) {
            ...
            return std::make_shared<Ewoms::ParallelOverlappingILU0<Matrix, Vector, Vector, Comm>>(
                op.getmat(), comm, w, Ewoms::MILU_VARIANT::ILU, num_interior, redblack, reorder_spheres);
        } else {
            return std::make_shared<Ewoms::ParallelOverlappingILU0<Matrix, Vector, Vector, Comm>>(
                op.getmat(), comm, ilulevel, w, Ewoms::MILU_VARIANT::ILU, redblack, reorder_spheres);
        }
    }
    ...
    void addStandardPreconditioners(const Dune::Amg::SequentialInformation*)
    {
        ...
        doAddCreator("ILU0", [](const O& op, const P& prm, const std::function<Vector()>&) {
            const double w = prm.get<double>("relaxation", 1.0);
            return std::make_shared<Ewoms::ParallelOverlappingILU0<M, V, V>>(
                op.getmat(), 0, w, Ewoms::MILU_VARIANT::ILU);
        });
        doAddCreator("ParOverILU0", [](const O& op, const P& prm, const std::function<Vector()>&) {
            const double w = prm.get<double>("relaxation", 1.0);
            const int n = prm.get<int>("ilulevel", 0);
            return std::make_shared<Ewoms::ParallelOverlappingILU0<M, V, V>>(
                op.getmat(), n, w, Ewoms::MILU_VARIANT::ILU);
        });
                     ...
                     const std::function<Vector()> weightsCalculator)
    {
        ...
        if (it == creators_.end()) {
            ...
            msg << std::endl;
            EWOMS_THROW(std::invalid_argument, msg.str());
        }
...
#endif // EWOMS_PRECONDITIONERFACTORY_HEADER
