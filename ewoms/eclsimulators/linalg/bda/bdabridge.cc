...
void getSparsityPattern(BridgeMatrix& mat, std::vector<int> &h_rows, std::vector<int> &h_cols) {
    ...
    if (h_rows.size() == 0) {
        ...
        if (static_cast<unsigned int>(h_rows[mat.N()]) != mat.nonzeroes()) {
            EWOMS_THROW(std::logic_error, "Error size of rows do not sum to number of nonzeroes in BdaBridge::getSparsityPattern()");
        }
    }
...
(Dune::BCRSMatrix<Ewoms::MatrixBlock<double, n, n>, std::allocator<Ewoms::MatrixBlock<double, n, n> > >*, \
