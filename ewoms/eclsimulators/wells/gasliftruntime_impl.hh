...
GasLiftRuntime(
    ...
    eebos_simulator_{eebos_simulator},
    ...
    debug{false}  // extra debugging output
{
    ...
    const Ewoms::Schedule& schedule = this->eebos_simulator_.vanguard().schedule();
    const int report_step_idx = this->eebos_simulator_.episodeIndex();
...
computeInitialWellRates_()
{
    ...
    this->std_well_.computeWellRatesWithThpAlqProd(
        this->eebos_simulator_, this->summary_state_, this->deferred_logger_,
...
computeWellRates_(double bhp, std::vector<double> &potentials)
{
    this->std_well_.computeWellRatesWithBhp(
        this->eebos_simulator_, bhp, potentials, this->deferred_logger_);
...
template<typename TypeTag>
double
Ewoms::GasLiftRuntime<TypeTag>::OptimizeState::
