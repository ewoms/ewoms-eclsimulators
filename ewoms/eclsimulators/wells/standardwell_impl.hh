...
namespace Ewoms
{
    ...
    wellVolumeFractionScaled(const int compIdx) const
    {
        ...
        const int legacyCompIdx = eebosCompIdxToEFlowCompIdx(compIdx);
                    ...
                    Ewoms::DeferredLogger& deferred_logger) const
    {
        ...
        if ( drawdown.value() > 0 )  {
            ...
            if (FluidSystem::phaseIsActive(FluidSystem::oilPhaseIdx) && FluidSystem::phaseIsActive(FluidSystem::gasPhaseIdx)) {
                ...
                if (d.value() == 0.0) {
                    EWOMS_DEFLOG_THROW(Ewoms::NumericalIssue, "Zero d value obtained for well " << name() << " during flux calcuation"
                                   ...
                                   Ewoms::DeferredLogger& deferred_logger)
    {
        ...
        checkWellOperability(eebosSimulator, well_state, deferred_logger);
        ...
        assembleWellEqWithoutIterationImpl(eebosSimulator, dt, well_state, deferred_logger);
    }

    template<typename TypeTag>
    void
    StandardWell<TypeTag>::
    assembleWellEqWithoutIterationImpl(const Simulator& eebosSimulator,
                                       const double dt,
                                       WellState& well_state,
                                       Ewoms::DeferredLogger& deferred_logger)
    {
        ...
        for (int perf = 0; perf < number_of_perforations_; ++perf) {
            ...
            calculateSinglePerf(eebosSimulator, perf, well_state, connectionRates, cq_s, water_flux_s, cq_s_zfrac_effective, deferred_logger);

            // Equation assembly for this perforation.
            if (has_polymer && this->has_polymermw && this->isInjector()) {
                handleInjectivityEquations(eebosSimulator, well_state, perf, water_flux_s, deferred_logger);
            }
            const int cell_idx = well_cells_[perf];
            for (int componentIdx = 0; componentIdx < num_components_; ++componentIdx) {
                ...
                if (has_solvent && componentIdx == contiSolventEqIdx) {
                    ...
                    well_state.perfPhaseRates()[(first_perf_ + perf) * np + eebosCompIdxToEFlowCompIdx(componentIdx)] = cq_s[componentIdx].value();
                }
            }
        ...
        const Ewoms::Schedule& schedule = eebosSimulator.vanguard().schedule();
        ...
        try {
            ...
            EWOMS_DEFLOG_THROW(Ewoms::NumericalIssue,"Error when inverting local well equations for well " + name(), deferred_logger);
        }
    }
    ...
    void
    StandardWell<TypeTag>::
    calculateSinglePerf(const Simulator& eebosSimulator,
                        const int perf,
                        WellState& well_state,
                        ...
                        Ewoms::DeferredLogger& deferred_logger) const
    {
        ...
        if (has_polymer && this->has_polymermw && this->isInjector()) {
            ...
            handleInjectivityRate(eebosSimulator, perf, cq_s);
        }
        ...
        if (has_energy) {
            ...
            for (unsigned phaseIdx = 0; phaseIdx < FluidSystem::numPhases; ++phaseIdx) {
                ...
                if (this->isInjector() && cq_s[activeCompIdx] > 0.0){
                    ...
                    paramCache.setMaxOilSat(eebosSimulator.problem().maxOilSaturation(cell_idx));
    ...
    void
    StandardWell<TypeTag>::
    getMobility(const Simulator& eebosSimulator,
                const int perf,
                ...
                Ewoms::DeferredLogger& deferred_logger) const
    {
        ...
        const auto& intQuants = *(eebosSimulator.model().cachedIntensiveQuantities(cell_idx, /*timeIdx=*/0));
        const auto& materialLawManager = eebosSimulator.problem().materialLawManager();
        ...
        if( satid == satid_elem ) { // the same saturation number is used. i.e. just use the mobilty from the cell
            ...
            if (has_solvent) {
                EWOMS_DEFLOG_THROW(std::runtime_error, "individual mobility for wells does not work in combination with solvent", deferred_logger);
            }
        }
        ...
        if (has_polymer) {
            if (!FluidSystem::phaseIsActive(FluidSystem::waterPhaseIdx)) {
                EWOMS_DEFLOG_THROW(std::runtime_error, "Water is required when polymer is active", deferred_logger);
            }

            updateWaterMobilityWithPolymer(eebosSimulator, perf, mob, deferred_logger);
        }
    }
    ...
    void
    StandardWell<TypeTag>::
    updateThp(WellState& well_state, Ewoms::DeferredLogger& deferred_logger) const
    {
        ...
        const Ewoms::PhaseUsage& pu = phaseUsage();
        if (FluidSystem::phaseIsActive(FluidSystem::waterPhaseIdx)) {
    ...
    void
    StandardWell<TypeTag>::
    updateWellStateWithTarget(const Simulator& eebos_simulator,
                              WellState& well_state,
                              Ewoms::DeferredLogger& deferred_logger) const
    {
        ...
        const auto& summaryState = eebos_simulator.vanguard().summaryState();
        ...
        if (this->isInjector() )
        {
            ...
            {
                EWOMS_DEFLOG_THROW(std::runtime_error, "Well control must be specified for well "  + name(), deferred_logger );
            }
        ...
        else
        {
            ...
            {
                EWOMS_DEFLOG_THROW(std::runtime_error, "CRAT control not supported " << name(), deferred_logger);
            }
            ...
            case Well::ProducerCMode::THP:
            {
                ...
                gliftDebug(
                    ...
                    deferred_logger);
                auto bhp = computeBhpAtThpLimitProd(well_state, eebos_simulator, summaryState, deferred_logger);
                if (bhp) {
                    well_state.bhp()[well_index] = *bhp;
                } else {
    ...
    void
    StandardWell<TypeTag>::
    updateIPR(const Simulator& eebos_simulator, Ewoms::DeferredLogger& deferred_logger) const
    {
        ...
        for (int perf = 0; perf < number_of_perforations_; ++perf) {
            ...
            getMobility(eebos_simulator, perf, mob, deferred_logger);

            const int cell_idx = well_cells_[perf];
            const auto& int_quantities = *(eebos_simulator.model().cachedIntensiveQuantities(cell_idx, /*timeIdx=*/ 0));
            ...
            const double tw_perf = well_index_[perf]*eebos_simulator.problem().template rockCompTransMultiplier<double>(int_quantities, cell_idx);
            ...
            for (int p = 0; p < number_of_phases_; ++p) {
                ...
                ipr_a_[eebosCompIdxToEFlowCompIdx(p)] += ipr_a_perf[p];
                ipr_b_[eebosCompIdxToEFlowCompIdx(p)] += ipr_b_perf[p];
            }
        }
    }
    ...
    void
    StandardWell<TypeTag>::
    checkWellOperability(const Simulator& eebos_simulator,
                         ...
                         Ewoms::DeferredLogger& deferred_logger)
    {
        ...
        updateWellOperability(eebos_simulator, well_state, deferred_logger);
    ...
    void
    StandardWell<TypeTag>::
    updateWellOperability(const Simulator& eebos_simulator,
                          const WellState& well_state,
                          Ewoms::DeferredLogger& deferred_logger)
    {
        this->operability_status_.reset();

        updateIPR(eebos_simulator, deferred_logger);

        // checking the BHP limit related
        checkOperabilityUnderBHPLimitProducer(well_state, eebos_simulator, deferred_logger);

        const auto& summaryState = eebos_simulator.vanguard().summaryState();

        // checking whether the well can operate under the THP constraints.
        if (this->wellHasTHPConstraints(summaryState)) {
            checkOperabilityUnderTHPLimitProducer(eebos_simulator, well_state, deferred_logger);
        }
    }
    ...
    void
    StandardWell<TypeTag>::
    checkOperabilityUnderBHPLimitProducer(const WellState& well_state, const Simulator& eebos_simulator, Ewoms::DeferredLogger& deferred_logger)
    {
        const auto& summaryState = eebos_simulator.vanguard().summaryState();
        ...
        if ( bhp_limit_not_defaulted || !this->wellHasTHPConstraints(summaryState) ) {
            ...
            if (this->operability_status_.operable_under_only_bhp_limit && this->wellHasTHPConstraints(summaryState)) {
                ...
                computeWellRatesWithBhp(eebos_simulator, bhp_limit, well_rates_bhp_limit, deferred_logger);
    ...
    void
    StandardWell<TypeTag>::
    checkOperabilityUnderTHPLimitProducer(const Simulator& eebos_simulator, const WellState& well_state, Ewoms::DeferredLogger& deferred_logger)
    {
        const auto& summaryState = eebos_simulator.vanguard().summaryState();
        const auto obtain_bhp = computeBhpAtThpLimitProd(well_state, eebos_simulator, summaryState, deferred_logger);
    ...
    bool
    StandardWell<TypeTag>::
    allDrawDownWrongDirection(const Simulator& eebos_simulator) const
    {
    ...
    bool
    StandardWell<TypeTag>::
    canProduceInjectWithCurrentBhp(const Simulator& eebos_simulator,
                                   const WellState& well_state,
                                   Ewoms::DeferredLogger& deferred_logger)
    {
        const double bhp = well_state.bhp()[index_of_well_];
        std::vector<double> well_rates;
        computeWellRatesWithBhp(eebos_simulator, bhp, well_rates, deferred_logger);
    ...
    bool
    StandardWell<TypeTag>::
    openCrossFlowAvoidSingularity(const Simulator& eebos_simulator) const
    {
        return !getAllowCrossFlow() && allDrawDownWrongDirection(eebos_simulator);
    }

    template<typename TypeTag>
    void
    StandardWell<TypeTag>::
    computePropertiesForWellConnectionPressures(const Simulator& eebosSimulator,
                                                const WellState& well_state,
                            ...
                            DeferredLogger& deferred_logger) const
    {
        ...
        for (auto allPerfID = 0*nPerf; allPerfID < nPerf; ++allPerfID) {
            ...
            std::vector<EvalWell> mob(num_components_, {numWellEq_ + numEq, 0.0});
            getMobility(eebosSimulator, static_cast<int>(subsetPerfID), mob, deferred_logger);
                                           ...
                                           const std::vector<double>& surf_dens_perf)
    {
        ...
        for (int perf = 0; perf < nperf; ++perf) {
            for (int comp = 0; comp < np; ++comp) {
                perfRates[perf * num_components_ + comp] =  well_state.perfPhaseRates()[(first_perf_ + perf) * np + eebosCompIdxToEFlowCompIdx(comp)];
            }
            if(has_solvent) {
    ...
    void
    StandardWell<TypeTag>::
    computeWellConnectionPressures(const Simulator& eebosSimulator,
                                   const WellState& well_state)
    {
         ...
         computePropertiesForWellConnectionPressures(eebosSimulator, well_state, b_perf, rsmax_perf, rvmax_perf, surf_dens_perf);
    ...
    void
    StandardWell<TypeTag>::
    solveEqAndUpdateWellState(WellState& well_state, Ewoms::DeferredLogger& deferred_logger)
    {
        if (!this->isOperable() && !this->wellIsStopped()) return;
    ...
    void
    StandardWell<TypeTag>::
    calculateExplicitQuantities(const Simulator& eebosSimulator,
                                const WellState& well_state,
                                Ewoms::DeferredLogger& deferred_logger)
    {
        updatePrimaryVariables(well_state, deferred_logger);
        initPrimaryVariablesEvaluation();
        computeWellConnectionPressures(eebosSimulator, well_state);
        computeAccumWell();
    }
    ...
    void
    StandardWell<TypeTag>::
    computeWellRatesWithBhp(const Simulator& eebosSimulator,
                            const double& bhp,
                            ...
                            Ewoms::DeferredLogger& deferred_logger) const
    {
        ...
        for (int perf = 0; perf < number_of_perforations_; ++perf) {
            ...
            for(int p = 0; p < np; ++p) {
                well_flux[eebosCompIdxToEFlowCompIdx(p)] += cq_s[p].value();
            }
        }
    }
    ...
    void
    StandardWell<TypeTag>::
    computeWellRatesWithBhpPotential(const Simulator& eebosSimulator,
                            const std::vector<Scalar>& B_avg,
                            const double& bhp,
                            std::vector<double>& well_flux,
                            Ewoms::DeferredLogger& deferred_logger)
    {
        ...
        WellState well_state_copy = eebosSimulator.problem().wellModel().wellState();
        ...
        computeWellConnectionPressures(eebosSimulator, well_state_copy);
        initPrimaryVariablesEvaluation();

        computeWellRatesWithBhp(eebosSimulator, bhp, well_flux, deferred_logger);
    }
                               ...
                               const WellState &well_state) const
    {
        ...
        const auto& summary_state = eebos_simulator.vanguard().summaryState();

        const auto& well = well_ecl_;
        if (well.isInjector()){
            const auto& controls = well_ecl_.injectionControls(summary_state);
            auto bhp_at_thp_limit = computeBhpAtThpLimitInj(eebos_simulator, summary_state, deferred_logger);
            if (bhp_at_thp_limit) {
                const double bhp = std::min(*bhp_at_thp_limit, controls.bhp_limit);
                computeWellRatesWithBhp(eebos_simulator, bhp, potentials, deferred_logger);
            } else {
    ...
    bool
    StandardWell<TypeTag>::
    doGasLiftOptimize(const WellState &well_state, const Simulator &eebos_simulator,
                      Ewoms::DeferredLogger& deferred_logger) const
    {
        ...
        if (!checkGliftNewtonIterationIdxOk(eebos_simulator, deferred_logger)) {
            return false;
        }
        const int report_step_idx = eebos_simulator.episodeIndex();
        const Ewoms::Schedule& schedule = eebos_simulator.vanguard().schedule();
            ...
            DeferredLogger& deferred_logger ) const
    {
        ...
        const Ewoms::Schedule& schedule = eebos_simulator.vanguard().schedule();
        const GasLiftOpt& glo = schedule.glo(report_step_idx);
        const int iteration_idx = eebos_simulator.model().newtonMethod().numIterations();
        if (glo.all_newton()) {
    ...
    double
    StandardWell<TypeTag>::
    computeWellRatesAndBhpWithThpAlqProd(const Simulator &eebos_simulator,
                               ...
                               double alq) const
    {
        ...
        auto bhp_at_thp_limit = computeBhpAtThpLimitProdWithAlq(
                              eebos_simulator, summary_state, deferred_logger, alq);
        if (bhp_at_thp_limit) {
    ...
    void
    StandardWell<TypeTag>::
    computeWellRatesWithThpAlqProd(const Simulator &eebos_simulator,
                               const SummaryState &summary_state,
                               DeferredLogger &deferred_logger,
                               std::vector<double> &potentials,
                               double alq) const
    ...
    void
    StandardWell<TypeTag>::
    computeWellPotentials(const Simulator& eebosSimulator,
                          const std::vector<Scalar>& B_avg,
                          const WellState& well_state,
                          ...
                          Ewoms::DeferredLogger& deferred_logger) // const
    {
        ...
        if ( !well.Base::wellHasTHPConstraints(summaryState) || current_control == Well::ProducerCMode::BHP ) {
            ...
            well.computeWellRatesWithBhpPotential(eebosSimulator, B_avg, bhp, well_potentials, deferred_logger);
        } else {
            // the well has a THP related constraint
            well_potentials = well.computeWellPotentialWithTHP(eebosSimulator, deferred_logger, well_state);
        }
    }
    ...
    void
    StandardWell<TypeTag>::
    updatePrimaryVariables(const WellState& well_state, Ewoms::DeferredLogger& deferred_logger) const
    {
        if (!this->isOperable() && !this->wellIsStopped()) return;
        ...
        if (std::abs(total_well_rate) > 0.) {
            ...
            if (this->isInjector()) {
                ...
                EWOMS_DEFLOG_THROW(std::logic_error, "Expected PRODUCER or INJECTOR type of well", deferred_logger);
            }
        }
    ...
    void
    StandardWell<TypeTag>::
    updateWaterMobilityWithPolymer(const Simulator& eebos_simulator,
                                   const int perf,
                                   std::vector<EvalWell>& mob,
                                   Ewoms::DeferredLogger& deferred_logger) const
    {
        ...
        const int cell_idx = well_cells_[perf];
        const auto& int_quant = *(eebos_simulator.model().cachedIntensiveQuantities(cell_idx, /*timeIdx=*/ 0));
        ...
        if (PolymerModule::hasPlyshlog()) {
            ...
            const bool allow_cf = getAllowCrossFlow() || openCrossFlowAvoidSingularity(eebos_simulator);
            const EvalWell& bhp = getBhp();

            std::vector<EvalWell> cq_s(num_components_, {numWellEq_ + numEq, 0.});
            double perf_dis_gas_rate = 0.;
            double perf_vap_oil_rate = 0.;
            double trans_mult = eebos_simulator.problem().template rockCompTransMultiplier<double>(int_quant, cell_idx);
            const double Tw = well_index_[perf] * trans_mult;
            computePerfRate(int_quant, mob, bhp, Tw, perf, allow_cf,
                            cq_s, perf_dis_gas_rate, perf_vap_oil_rate, deferred_logger);
            // TODO: make area a member
            const double area = 2 * M_PI * perf_rep_radius_[perf] * perf_length_[perf];
            const auto& material_law_manager = eebos_simulator.problem().materialLawManager();
            ...
            const EvalWell denom = Ewoms::max( (area * poro * (sw - swcr)), 1e-12);
            const unsigned waterCompIdx = Indices::canonicalToActiveComponentIndex(FluidSystem::waterCompIdx);
    ...
    void
    StandardWell<TypeTag>::
    wellTestingPhysical(const Simulator& eebos_simulator, const std::vector<double>& B_avg,
                        ...
                        Ewoms::DeferredLogger& deferred_logger)
    {
        ...
        updateWellOperability(eebos_simulator, well_state_copy, deferred_logger);
    ...
    {
        if (!this->has_polymermw) {
            EWOMS_DEFLOG_THROW(std::runtime_error, "Polymermw is not activated, "
              ...
              Ewoms::DeferredLogger& deferred_logger) const
    {
        ...
        const EvalWell water_velocity_abs = Ewoms::abs(water_velocity);
        if (poly_inj_conc == 0.) {
        ...
        if (polymer_table_id <= 0) {
            EWOMS_DEFLOG_THROW(std::runtime_error, "Unavailable SKPRPOLY table id used for well " << name(), deferred_logger);
        }
    ...
    void
    StandardWell<TypeTag>::
    handleInjectivityRate(const Simulator& eebosSimulator,
                          const int perf,
                          std::vector<EvalWell>& cq_s) const
    {
        const int cell_idx = well_cells_[perf];
        const auto& int_quants = *(eebosSimulator.model().cachedIntensiveQuantities(cell_idx, /*timeIdx=*/ 0));
    ...
    void
    StandardWell<TypeTag>::
    handleInjectivityEquations(const Simulator& eebosSimulator,
                               const WellState& well_state,
                               const int perf,
                              ...
                              DeferredLogger& deferred_logger) const
    {
        ...
        else if (this->isInjector() )
        {
            ...
            case Well::InjectorCMode::GRUP:
                ...
                break;
            default:
                EWOMS_DEFLOG_THROW(std::runtime_error, "Unknown well control control types for well " << name(), deferred_logger);
            }
        }
        else if (this->isProducer() )
        {
                             ...
                             DeferredLogger& deferred_logger) const
    {
        return computeBhpAtThpLimitProdWithAlq(
            eebos_simulator, summary_state, deferred_logger, getALQ(well_state));
    }
                                    ...
                                    double alq_value) const
    {
        ...
        auto frates = [this, &eebos_simulator, &deferred_logger](const double bhp) {
            ...
            computeWellRatesWithBhp(eebos_simulator, bhp, rates, deferred_logger);
            return rates;
        };
...
// #define EXTRA_THP_DEBUGGING
                            ...
                            double* connPI) const
    {
        ...
        for (int p = 0; p < np; ++p) {
            ...
            // the reciprocal FVF.
            const auto connMob =
                mobility[ eflowPhaseToEebosCompIdx(p) ].value() * fs.invB(p).value();
