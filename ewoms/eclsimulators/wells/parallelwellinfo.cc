...
{
public:
    typedef typename Dune::CommPolicy<CommType>::IndexedType IndexType;

    static IndexType gather(const CommType& dataArray, std::size_t idx)
    { return dataArray[idx]; }

    static void scatter(CommType& dataArray, IndexType value, std::size_t idx)
    { dataArray[idx] = value; }
};
#endif
...
struct MySillyOtherCopyGatherScatter
{
                                                  ...
                                                  std::size_t size)
{
    ...
    if (comm_.size() > 1)
    {
        ...
        communicator_.forward<MySillyOtherCopyGatherScatter>(const_cast<double*>(current), aboveData);
    }
    else
#endif
    {
