...
namespace Ewoms
{
    ...
    , segment_phase_densities_(numberOfSegments(), std::vector<EvalWell>(num_components_, 0.0)) // number of phase here?
    {
        ...
        if (has_solvent) {
            EWOMS_THROW(std::runtime_error, "solvent is not supported by multisegment well yet");
        }

        if (has_polymer) {
            EWOMS_THROW(std::runtime_error, "polymer is not supported by multisegment well yet");
        }
         ...
         const std::vector< Scalar >& B_avg)
    {
        ...
        // An indicator from the ewoms-eclio should indicate what kind of depth we should use here.
    ...
    ConvergenceReport
    MultisegmentWell<TypeTag>::
    getWellConvergence(const WellState& well_state, const std::vector<double>& B_avg, Ewoms::DeferredLogger& deferred_logger, const bool relax_tolerance) const
    {
...
/*         {
        ...
        if ( !well.Base::wellHasTHPConstraints(summaryState) || current_control == Well::ProducerCMode::BHP) {
            well.computeWellRatesAtBhpLimit(eebosSimulator, B_avg, well_potentials, deferred_logger);
        } else {
            well_potentials = well.computeWellPotentialWithTHP(eebosSimulator, B_avg, deferred_logger);
        }
    ...
    void
    MultisegmentWell<TypeTag>::
    computeWellRatesAtBhpLimit(const Simulator& eebosSimulator,
                               const std::vector<Scalar>& B_avg,
                               std::vector<double>& well_flux,
                               Ewoms::DeferredLogger& deferred_logger) const
    {
        if (well_ecl_.isInjector()) {
            const auto controls = well_ecl_.injectionControls(eebosSimulator.vanguard().summaryState());
            computeWellRatesWithBhp(eebosSimulator, B_avg, controls.bhp_limit, well_flux, deferred_logger);
        } else {
            const auto controls = well_ecl_.productionControls(eebosSimulator.vanguard().summaryState());
            computeWellRatesWithBhp(eebosSimulator, B_avg, controls.bhp_limit, well_flux, deferred_logger);
        }
    }
                                ...
                                Ewoms::DeferredLogger& deferred_logger) const
    {
        ...
        const auto& well = well_ecl_;
        if (well.isInjector()){
            auto bhp_at_thp_limit = computeBhpAtThpLimitInj(eebos_simulator, B_avg, summary_state, deferred_logger);
            if (bhp_at_thp_limit) {
                ...
                computeWellRatesWithBhp(eebos_simulator, B_avg, bhp, potentials, deferred_logger);
            ...
            auto bhp_at_thp_limit = computeBhpAtThpLimitProd(eebos_simulator, B_avg, summary_state, deferred_logger);
            if (bhp_at_thp_limit) {
    ...
    void
    MultisegmentWell<TypeTag>::
    computePerfCellPressDiffs(const Simulator& eebosSimulator)
    {
        for (int perf = 0; perf < number_of_perforations_; ++perf) {
    ...
    void
    MultisegmentWell<TypeTag>::
    computeInitialSegmentFluids(const Simulator& eebos_simulator)
    {
        for (int seg = 0; seg < numberOfSegments(); ++seg) {
            // TODO: trying to reduce the times for the surfaceVolumeFraction calculation
            const double surface_volume = getSegmentSurfaceVolume(eebos_simulator, seg).value();
                                ...
                                Ewoms::DeferredLogger& deferred_logger)
    {
        ...
        computePerfCellPressDiffs(eebosSimulator);
        computeInitialSegmentFluids(eebosSimulator);
    }
    ...
    volumeFractionScaled(const int seg, const int comp_idx) const
    {
        ...
        // phase is not zero.
        const double scale = scalingFactor(eebosCompIdxToEFlowCompIdx(comp_idx));
        if (scale > 0.) {
    ...
    void
    MultisegmentWell<TypeTag>::
    computeSegmentFluidProperties(const Simulator& eebosSimulator)
    {
    ...
    checkOperabilityUnderBHPLimitProducer(const WellState& /*well_state*/, const Simulator& eebos_simulator, Ewoms::DeferredLogger& deferred_logger)
    {
        ...
        if ( bhp_limit_not_defaulted || !this->wellHasTHPConstraints(summaryState) ) {
            ...
            if (this->operability_status_.operable_under_only_bhp_limit && this->wellHasTHPConstraints(summaryState)) {
                ...
                computeWellRatesWithBhp(eebos_simulator, Base::B_avg_, bhp_limit, well_rates_bhp_limit, deferred_logger);
    ...
    updateIPR(const Simulator& eebos_simulator, Ewoms::DeferredLogger& deferred_logger) const
    {
        ...
        for (int seg = 0; seg < nseg; ++seg) {
            ...
            getMobility(eebos_simulator, perf, mob);
    ...
    {
        const auto& summaryState = eebos_simulator.vanguard().summaryState();
        const auto obtain_bhp = computeBhpAtThpLimitProd(eebos_simulator, Base::B_avg_, summaryState, deferred_logger);
                                   ...
                                   Ewoms::DeferredLogger& deferred_logger)
    {
        ...
        computeSegmentFluidProperties(eebosSimulator);
        ...
        for (int seg = 0; seg < nseg; ++seg) {
            ...
            {
                const EvalWell segment_surface_volume = getSegmentSurfaceVolume(eebosSimulator, seg);
            ...
            for (const int perf : segment_perforations_[seg]) {
                ...
                std::vector<EvalWell> mob(num_components_, 0.0);
                getMobility(eebosSimulator, perf, mob);
                const double trans_mult = eebosSimulator.problem().template rockCompTransMultiplier<double>(int_quants, cell_idx);
                ...
                for (int comp_idx = 0; comp_idx < num_components_; ++comp_idx) {
                    well_state.perfPhaseRates()[rate_start_offset + eebosCompIdxToEFlowCompIdx(comp_idx)] = cq_s[comp_idx].value();
                }
            ...
            if (seg == 0) { // top segment, pressure equation is the control equation
                ...
                case Segment::SegmentType::AICD : {
                    const UnitSystem& unit_system = eebosSimulator.vanguard().eclState().getDeckUnitSystem();
    ...
    void
    MultisegmentWell<TypeTag>::
    updateWaterThroughput(const double dt EWOMS_UNUSED, WellState& well_state EWOMS_UNUSED) const
    {
    }
    ...
    getSegmentSurfaceVolume(const Simulator& eebos_simulator, const int seg_idx) const
    {
        ...
        if (FluidSystem::phaseIsActive(FluidSystem::oilPhaseIdx) && FluidSystem::phaseIsActive(FluidSystem::gasPhaseIdx)) {
            ...
            const EvalWell d = 1.0 - rs * rv;
            if (d <= 0.0 || d > 1.0) {
                EWOMS_THROW(Ewoms::NumericalIssue, "Problematic d value " << d << " obtained for well " << name()
    ...
    getWellResiduals(const std::vector<Scalar>& B_avg) const
    {
        ...
        for (int seg = 0; seg < numberOfSegments(); ++seg) {
            for (int eq_idx = 0; eq_idx < numWellEq; ++eq_idx) {
                ...
                if (std::isnan(residual) || std::isinf(residual)) {
                    EWOMS_THROW(Ewoms::NumericalIssue, "nan or inf value for residal get for well " << name()
        ...
        // handling the control equation residual
        {
            ...
            if (std::isnan(control_residual) || std::isinf(control_residual)) {
               EWOMS_THROW(Ewoms::NumericalIssue, "nan or inf value for control residal get for well " << name());
            }
                             ...
                             DeferredLogger& deferred_logger) const
    {
        ...
        auto frates = [this, &eebos_simulator, &B_avg, &deferred_logger](const double bhp) {
            ...
            computeWellRatesWithBhp(eebos_simulator, B_avg, bhp, rates, deferred_logger);
            return rates;
        };
        ...
        double bhp_max = 0.0;
        {
            ...
            double high = maxPerfPress(eebos_simulator) + 1.0 * unit::barsa;
...
// #define EXTRA_THP_DEBUGGING
    ...
    double
    MultisegmentWell<TypeTag>::
    maxPerfPress(const Simulator& eebos_simulator) const
    {
        ...
        for (int seg = 0; seg < nseg; ++seg) {
            for (const int perf : segment_perforations_[seg]) {
                const int cell_idx = well_cells_[perf];
                const auto& int_quants = *(eebos_simulator.model().cachedIntensiveQuantities(cell_idx, /*timeIdx=*/ 0));
                const auto& fs = int_quants.fluidState();
