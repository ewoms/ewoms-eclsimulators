...
#ifndef EWOMS_WELLSTATEFULLYIMPLICITBLACKOIL_HH
#define EWOMS_WELLSTATEFULLYIMPLICITBLACKOIL_HH

#include <ewoms/eclsimulators/wells/wellstate.hh>
...
namespace Ewoms
{
    ...
    public:
        ...
        std::vector<Ewoms::Well::InjectorCMode>& currentInjectionControls() { return current_injection_controls_; }
        const std::vector<Ewoms::Well::InjectorCMode>& currentInjectionControls() const { return current_injection_controls_; }
        ...
        bool hasInjectionGroupControl(const Ewoms::Phase& phase, const std::string& groupName) const {
        ...
        const Group::ProductionCMode& currentProductionGroupControl(const std::string& groupName) const {
            ...
            if (it == current_production_group_controls_.end())
                EWOMS_THROW(std::logic_error, "Could not find any control for production group " << groupName);
        ...
        const std::vector<double>& currentProductionGroupReductionRates(const std::string& groupName) const {
            ...
            if (it == production_group_reduction_rates.end())
                EWOMS_THROW(std::logic_error, "Could not find any reduction rates for production group  " << groupName);
        ...
        const std::vector<double>& currentInjectionGroupReductionRates(const std::string& groupName) const {
            ...
            if (it == injection_group_reduction_rates.end())
                EWOMS_THROW(std::logic_error, "Could not find any reduction rates for injection group " << groupName);
        ...
        const double& currentInjectionVREPRates(const std::string& groupName) const {
            ...
            if (it == injection_group_vrep_rates.end())
                EWOMS_THROW(std::logic_error, "Could not find any VREP rates for group " << groupName);
        ...
        const std::vector<double>& currentInjectionREINRates(const std::string& groupName) const {
            ...
            if (it == injection_group_rein_rates.end())
                EWOMS_THROW(std::logic_error, "Could not find any REIN rates for group " << groupName);
        ...
        data::Wells report(const PhaseUsage &pu, const int* globalCellIdxMap) const override
        {
             ...
             * WellState to something understood by ewoms-eclio. it is intended
    ...
    private:
        ...
        {
            auto seg_res = ::Ewoms::data::Segment{};
            ...
            {
                using Value =::Ewoms::data::SegmentPressures::Value;
...
#endif // EWOMS_WELLSTATEFULLYIMPLICITBLACKOIL_HH
