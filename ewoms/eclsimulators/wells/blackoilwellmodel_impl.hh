...
namespace Ewoms {
        ...
        , has_polymer_(GET_PROP_VALUE(TypeTag, EnablePolymer))
    {
        ...
        local_num_cells_ = eebosSimulator_.gridView().size(0);
        // Number of cells the global grid view
        global_num_cells_ = eebosSimulator_.vanguard().globalNumCells();
        ...
        auto& parallel_wells = eebosSimulator.vanguard().parallelWells();
        parallel_well_info_.assign(parallel_wells.begin(), parallel_wells.end());
        const auto& pwell_info = parallel_well_info_;
        std::size_t numProcs = eebosSimulator.gridView().comm().size();
    ...
    template<typename TypeTag>
    void
    BlackoilWellModel<TypeTag>::wellTesting(const int timeStepIdx, const double simulationTime, Ewoms::DeferredLogger& deferred_logger) {
    ...
    well(const std::string& wellName) const
    {
        ...
        EWOMS_THROW(std::invalid_argument, "The well with name " + wellName + " is not in the well Container");
        return nullptr;
    }
                          ...
                          Ewoms::DeferredLogger& deferred_logger) const
    {
        ...
        if (index_well_ecl == nw_wells_ecl) {
            EWOMS_DEFLOG_THROW(std::logic_error, "Could not find well " << well_name << " in wells_ecl ", deferred_logger);
        }
    ...
    void
    BlackoilWellModel<TypeTag>::
    calculateExplicitQuantities(Ewoms::DeferredLogger& deferred_logger) const
    {
    ...
    void
    BlackoilWellModel<TypeTag>::
    updateWellControls(Ewoms::DeferredLogger& deferred_logger, const bool checkGroupControls)
    {
        ...
        if (checkGroupControls) {
            ...
            for (const auto& well : well_container_) {
                ...
                const bool changed = well->updateWellControl(eebosSimulator_, mode, well_state_, deferred_logger);
                if (changed) {
        ...
        for (const auto& well : well_container_) {
            ...
            well->updateWellControl(eebosSimulator_, mode, well_state_, deferred_logger);
        }
        updateAndCommunicateGroupData();
    ...
    void
    BlackoilWellModel<TypeTag>::
    computeWellPotentials(std::vector<double>& well_potentials, const int reportStepIdx, Ewoms::DeferredLogger& deferred_logger)
    {
    ...
    void
    BlackoilWellModel<TypeTag>::
    prepareTimeStep(Ewoms::DeferredLogger& deferred_logger)
    {
        int exception_thrown = 0;
        try {
            for (const auto& well : well_container_) {
    ...
    const typename BlackoilWellModel<TypeTag>::WellState&
    BlackoilWellModel<TypeTag>::
    wellState(const WellState& well_state EWOMS_UNUSED) const { return wellState(); }
    ...
    setupCartesianToCompressed_(const int* global_cell, int number_of_cartesian_cells)
    {
        ...
        if (global_cell) {
            auto elemIt = eebosSimulator_.gridView().template begin</*codim=*/ 0>();
            for (unsigned i = 0; i < local_num_cells_; ++i) {
                // Skip perforations in the overlap/ghost for distributed wells.
                if (elemIt->partitionType() == Dune::InteriorEntity)
                {
                    assert(eebosSimulator_.gridView().indexSet().index(*elemIt) == static_cast<int>(i));
    ...
    void
    BlackoilWellModel<TypeTag>::
    computeRepRadiusPerfLength(const Grid& grid, Ewoms::DeferredLogger& deferred_logger)
    {
        for (const auto& well : well_container_) {
    ...
    void
    BlackoilWellModel<TypeTag>::
    updatePrimaryVariables(Ewoms::DeferredLogger& deferred_logger)
    {
        for (const auto& well : well_container_) {
    ...
    void
    BlackoilWellModel<TypeTag>::
    updateGroupIndividualControl(const Group& group, Ewoms::DeferredLogger& deferred_logger, std::set<std::string>& switched_groups) {
    ...
    bool
    BlackoilWellModel<TypeTag>::
    checkGroupConstraints(const Group& group, Ewoms::DeferredLogger& deferred_logger) const {
    ...
    BlackoilWellModel<TypeTag>::
    checkGroupProductionConstraints(const Group& group, Ewoms::DeferredLogger& deferred_logger) const {
        ...
        {
            EWOMS_DEFLOG_THROW(std::runtime_error, "Group " + group.name() + "CRAT control for production groups not implemented" , deferred_logger);
        }
        ...
        {
            EWOMS_DEFLOG_THROW(std::runtime_error, "Group " + group.name() + "PRBL control for production groups not implemented", deferred_logger);
        }
    ...
    checkGroupInjectionConstraints(const Group& group, const Phase& phase) const {
        ...
        if (phase == Phase::GAS && phase_usage_.phase_used[BlackoilPhases::Vapour] )
            ...
            EWOMS_THROW(std::runtime_error, "Unknown phase" );
    ...
    actionOnBrokenConstraints(const Group& group, const Group::ExceedAction& exceed_action, const Group::ProductionCMode& newControl, Ewoms::DeferredLogger& deferred_logger) {
        ...
        case Group::ExceedAction::CON: {
            EWOMS_DEFLOG_THROW(std::runtime_error, "Group " + group.name() + "GroupProductionExceedLimit CON not implemented", deferred_logger);
            break;
        }
        case Group::ExceedAction::CON_PLUS: {
            EWOMS_DEFLOG_THROW(std::runtime_error, "Group " + group.name() + "GroupProductionExceedLimit CON_PLUS not implemented", deferred_logger);
            break;
        }
        case Group::ExceedAction::WELL: {
            EWOMS_DEFLOG_THROW(std::runtime_error, "Group " + group.name() + "GroupProductionExceedLimit WELL not implemented", deferred_logger);
            break;
        }
        case Group::ExceedAction::PLUG: {
            EWOMS_DEFLOG_THROW(std::runtime_error, "Group " + group.name() + "GroupProductionExceedLimit PLUG not implemented", deferred_logger);
            break;
        }
    ...
    void
    BlackoilWellModel<TypeTag>::
    updateGroupHigherControls(Ewoms::DeferredLogger& deferred_logger, std::set<std::string>& switched_groups)
    {
        const int reportStepIdx = eebosSimulator_.episodeIndex();
        const Group& fieldGroup = schedule().getGroup("FIELD", reportStepIdx);
    ...
    void
    BlackoilWellModel<TypeTag>::
    checkGroupHigherConstraints(const Group& group, Ewoms::DeferredLogger& deferred_logger, std::set<std::string>& switched_groups)
    {
    ...
    {
        if (static_cast<bool>(this->last_run_wellpi_) && (*this->last_run_wellpi_ == timeStepIdx)) {
        ...
        {
            {
                ...
                auto& schedule = this->eebosSimulator_.vanguard().schedule(); // Mutable
