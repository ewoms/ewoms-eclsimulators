...
namespace {
    Ewoms::GuideRate::RateVector
    getGuideRateVector(const std::vector<double>& rates, const Ewoms::PhaseUsage& pu)
    {
        using Ewoms::BlackoilPhases;
...
#define EXTRA_DEBUG_NETWORK 0
    ...
    int FractionCalculator::groupControlledWells(const std::string& group_name,
                                                 const std::string& always_included_child)
    {
        return ::Ewoms::WellGroupHelpers::groupControlledWells(
