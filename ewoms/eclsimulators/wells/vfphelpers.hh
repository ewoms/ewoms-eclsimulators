...
#ifndef EWOMS_AUTODIFF_VFPHELPERS_HH_
#define EWOMS_AUTODIFF_VFPHELPERS_HH_
...
static T getFlo(const T& aqua, const T& liquid, const T& vapour,
    ...
    switch (type) {
        ...
        default:
            EWOMS_THROW(std::logic_error, "Invalid FLO_TYPE: '" << type << "'");
    }
}
...
static T getWFR(const T& aqua, const T& liquid, const T& vapour,
    ...
    switch(type) {
        ...
        default:
            EWOMS_THROW(std::logic_error, "Invalid WFR_TYPE: '" << type << "'");
    }
}
...
static T getGFR(const T& aqua, const T& liquid, const T& vapour,
    ...
    switch(type) {
        ...
        default:
            EWOMS_THROW(std::logic_error, "Invalid GFR_TYPE: '" << type << "'");
    }
}
...
inline VFPEvaluation bhp(const VFPProdTable* table,
    ...
    //Recall that flow is negative in eWoms, so switch the sign.
...
const T* getTable(const std::map<int, T*> tables, int table_id) {
    ...
    if (entry == tables.end()) {
        EWOMS_THROW(std::invalid_argument, "Nonexistent VFP table " << table_id << " referenced.");
    }
    else {
        return entry->second;
    }
...
inline double findTHP(
    ...
    //bhp_array not sorted, raw search.
        ...
        //Target bhp greater than all values in array, extrapolate
            ...
            EWOMS_THROW(std::logic_error, "Programmer error: Unable to find THP in THP array");
        }
    }
...
#endif /* EWOMS_AUTODIFF_VFPHELPERS_HH_ */
