...
#include <ewoms/eclsimulators/wells/targetcalculator.hh>
...
namespace Ewoms
{
    ...
    int
    WellInterface<TypeTag>::
    eflowPhaseToEebosCompIdx( const int phaseIdx ) const
    {
    ...
    int
    WellInterface<TypeTag>::
    eebosCompIdxToEFlowCompIdx( const unsigned compIdx ) const
    {
        const auto& pu = phaseUsage();
    ...
    bool
    WellInterface<TypeTag>::
    updateWellControl(const Simulator& eebos_simulator,
                      ...
                      Ewoms::DeferredLogger& deferred_logger) /* const */
    {
        ...
        const auto& schedule = eebos_simulator.vanguard().schedule();
        ...
        if (changed) {
            ...
            updateWellStateWithTarget(eebos_simulator, well_state, deferred_logger);
    ...
    {
        auto cell_to_faces = Ewoms::UgGridHelpers::cell2Faces(grid);
        auto begin_face_centroids = Ewoms::UgGridHelpers::beginFaceCentroids(grid);
            ...
            if (connection.state() == Connection::State::OPEN) {
                ...
                if (cell >= 0) {
                    ...
                    switch (connection.dir()) {
                        case Ewoms::Connection::Direction::X:
                            re = std::sqrt(cubical[1] * cubical[2] / M_PI);
                            perf_length = cubical[0];
                            break;
                        case Ewoms::Connection::Direction::Y:
                            re = std::sqrt(cubical[0] * cubical[2] / M_PI);
                            perf_length = cubical[1];
                            break;
                        case Ewoms::Connection::Direction::Z:
                            re = std::sqrt(cubical[0] * cubical[1] / M_PI);
                            perf_length = cubical[2];
                            break;
                        default:
                            EWOMS_DEFLOG_THROW(std::runtime_error, " Dirtecion of well is not supported ", deferred_logger);
                    }
    ...
    template<typename TypeTag>
    bool
    WellInterface<TypeTag>::isVFPActive(Ewoms::DeferredLogger& deferred_logger) const
    {
        ...
        if (isProducer()) { // producer
            ...
            if (table_id <= 0) {
                ...
                if (vfp_properties_->getProd()->hasTable(table_id)) {
                    ...
                    EWOMS_DEFLOG_THROW(std::runtime_error, "VFPPROD table " << std::to_string(table_id) << " is specfied,"
            ...
            if (table_id <= 0) {
                ...
                if (vfp_properties_->getInj()->hasTable(table_id)) {
                    ...
                    EWOMS_DEFLOG_THROW(std::runtime_error, "VFPINJ table " << std::to_string(table_id) << " is specfied,"
                              << " for well " << name() << ", while we could not access it during simulation", deferred_logger);
                }
            }
        }
    ...
    void
    WellInterface<TypeTag>::
    solveWellForTesting(const Simulator& eebosSimulator, WellState& well_state,
                        const std::vector<double>& B_avg,
                        Ewoms::DeferredLogger& deferred_logger)
    {
    ...
    WellInterface<TypeTag>::volumetricSurfaceRateForConnection(int cellIdx, int phaseIdx) const {
        ...
        EWOMS_THROW(std::invalid_argument, "The well with name " + name()
                                                       ...
                                                       const SummaryState& summaryState) const
    {
        ...
        if (well.isInjector()) {
            ...
            Ewoms::Well::InjectorCMode& currentControl = well_state.currentInjectionControls()[well_index];
