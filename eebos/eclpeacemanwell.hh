...
class EclPeacemanWell : public BaseAuxiliaryModule<TypeTag>
{
    ...
    // tested with Ewoms::FluidSystems::BlackOil...
    ...
    typedef Ewoms::CompositionalFluidState<Scalar, FluidSystem, /*storeEnthalpy=*/true> FluidState;
    ...
    struct DofVariables {
        ...
        void updateBeginTimestep(const IntensiveQuantities& intQuants EWOMS_UNUSED)
        {}
...
public:
    ...
    virtual void applyInitial()
    {
        ...
        Ewoms::Valgrind::SetDefined(sol[wellGlobalDof]);
    ...
    virtual void linearize(SparseMatrixAdapter& matrix, GlobalEqVector& residual)
    {
        ...
        for (; wellDofIt != wellDofEndIt; ++ wellDofIt) {
            ...
            // problem...
            Ewoms::Valgrind::CheckDefined(q);
            block = 0.0;
            for (unsigned eqIdx = 0; eqIdx < numModelEq; ++ eqIdx)
                block[eqIdx][0] = - Ewoms::getValue(q[eqIdx])/dofVars.totalVolume;
...
protected:
    ...
    {
        typedef Ewoms::MathToolbox<Evaluation> DofVarsToolbox;
        ...
        for (unsigned phaseIdx = 0; phaseIdx < numPhases; ++phaseIdx) {
            ...
            Ewoms::Valgrind::CheckDefined(pbh);
            Ewoms::Valgrind::CheckDefined(p);
            Ewoms::Valgrind::CheckDefined(g);
            Ewoms::Valgrind::CheckDefined(rho);
            Ewoms::Valgrind::CheckDefined(lambda);
            Ewoms::Valgrind::CheckDefined(depth);
            Ewoms::Valgrind::CheckDefined(refDepth_);
            ...
            Ewoms::Valgrind::CheckDefined(g);
            Ewoms::Valgrind::CheckDefined(ph);
            Ewoms::Valgrind::CheckDefined(volRates[phaseIdx]);
        }
    }
                              ...
                              const DofVariables& dofVars) const
    {
        ...
        // system. Currently, only Ewoms::FluidSystems::BlackOil is supported, sorry...
    ...
    Scalar computeRateEquivalentBhp_() const
    {
        ...
        typedef Ewoms::DenseAd::Evaluation<Scalar, 1> BhpEval;
        ...
        for (int iterNum = 0; iterNum < maxIter; ++iterNum) {
            ...
            if (std::abs(f.derivative(0)) < 1e-20)
                throw Ewoms::NumericalIssue("Cannot determine the bottom hole pressure for well "+name()
        ...
        throw Ewoms::NumericalIssue("Could not determine the bottom hole pressure of well '"+name()
    ...
    {
        typedef Ewoms::MathToolbox<BhpEval> BhpEvalToolbox;
        ...
        Ewoms::Valgrind::CheckDefined(maximumSurfaceRate_);
        Ewoms::Valgrind::CheckDefined(maximumReservoirRate_);
        Ewoms::Valgrind::CheckDefined(surfaceRate);
        Ewoms::Valgrind::CheckDefined(resvRate);
