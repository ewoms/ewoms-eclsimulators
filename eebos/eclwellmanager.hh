...
class EclWellManager
{
    ...
    typedef std::map<int, std::pair<const Ewoms::Connection*, std::shared_ptr<Well> > > WellConnectionsMap;
...
public:
    ...
    void init()
    {
        const Ewoms::Schedule& deckSchedule = simulator_.vanguard().schedule();
        ...
        {
            const Ewoms::Well deckWell = deckSchedule.getWellsatEnd()[deckWellIdx];
    ...
    void beginEpisode(bool wasRestarted=false)
    {
        ...
        const std::vector<Ewoms::Well>& deckWells = deckSchedule.getWells(episodeIdx);
     ...
     */
    bool wellsChanged(const Ewoms::EclipseState& eclState,
                      ...
                      unsigned reportStepIdx) const
    {
        ...
        const Ewoms::Events& events = schedule.getEvents();
        return events.hasEvent(Ewoms::ScheduleEvents::PRODUCTION_UPDATE |
                               Ewoms::ScheduleEvents::INJECTION_UPDATE |
                               Ewoms::ScheduleEvents::WELL_STATUS_CHANGE,
                               reportStepIdx);
    }

    void initFromRestartFile(const RestartValue& restartValues EWOMS_UNUSED){
        // not implemented
    }

protected:
    bool wellTopologyChanged_(const Ewoms::EclipseState& eclState EWOMS_UNUSED,
                              const Ewoms::Schedule& schedule,
                              unsigned reportStepIdx) const
    {
        if (reportStepIdx == 0) {
        ...
        const Ewoms::Events& events = schedule.getEvents();
        return events.hasEvent(Ewoms::ScheduleEvents::NEW_WELL |
                               Ewoms::ScheduleEvents::COMPLETION_CHANGE,
                               reportStepIdx);
    }

    void updateWellTopology_(unsigned reportStepIdx EWOMS_UNUSED,
    ...
    void computeWellConnectionsMap_(unsigned reportStepIdx EWOMS_UNUSED, WellConnectionsMap& cartesianIdxToConnectionMap)
    {
    ...
    void updateWellParameters_(unsigned reportStepIdx, const WellConnectionsMap& wellConnections)
    {
        ...
        for (; elemIt != elemEndIt; ++elemIt) {
            ...
            for (unsigned dofIdx = 0; dofIdx < elemCtx.numPrimaryDof(/*timeIdx=*/0); ++ dofIdx)
            {
                ...
                const Ewoms::Connection* connection = connInfo.first;
