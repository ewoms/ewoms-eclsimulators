            ...
            std::vector<double>& p)
{
         ...
         ci != ce; ++ci, ++c)
    {
        ...
        const double z = Ewoms::UgGridHelpers::cellCenterDepth(grid, *ci);
             ...
             ci != ce; ++ci, ++ncell)
        {
                 ...
                 ++fi)
            {
                for (auto i = faceVertices[*fi].begin(), e = faceVertices[*fi].end();
                     i != e; ++i)
                {
                    const double z = Ewoms::UgGridHelpers::vertexCoordinates(grid, *i)[nd-1];
 ...
 *
 * \tparam FluidSystem  The FluidSystem from ewoms-material
 ...
 *                const_iterator, to traverse the range.
 *
 * \tparam MaterialLawManager The MaterialLawManager from ewoms-material
 *
 * \param[in] grid               Grid.
 * \param[in] reg             Current equilibration region.
 * \param[in] cells           Range that spans the cells of the current
 *                            equilibration region.
 * \param[in] materialLawManager   The MaterialLawManager from ewoms-material
    ...
    for (typename CellRange::const_iterator ci = cells.begin(); ci != cells.end(); ++ci, ++localIndex) {
        ...
        if (water) {
            if (isConstPc<FluidSystem, MaterialLaw, MaterialLawManager>(materialLawManager,FluidSystem::waterPhaseIdx, cell)){
                const double cellDepth = Ewoms::UgGridHelpers::cellCenterDepth(grid,
                                                                             cell);
                              ...
                              const std::vector<double> gasSaturation)
{
    ...
    for (auto it = cells.begin(); it != cells.end(); ++it, ++count) {
        const double depth = Ewoms::UgGridHelpers::cellCenterDepth(grid, *it);
...
private:
    ...
    void calcPressSatRsRv(const Ewoms::EclipseState& eclState EWOMS_UNUSED,
                          const RMap& reg,
