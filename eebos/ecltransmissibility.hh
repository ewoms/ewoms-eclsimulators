...
public:
    ...
    EclTransmissibility(const Vanguard& vanguard)
        : vanguard_(vanguard)
    {
        const Ewoms::UnitSystem& unitSystem = vanguard_.eclState().getDeckUnitSystem();
    ...
    void update(bool global)
    {
        ...
        if (comm.rank() == 0) {
            const auto& eclGrid = eclState.getInputGrid();
            useSmallestMultiplier = eclGrid.getMultzOption() == Ewoms::PinchMode::ModeEnum::ALL;
        }
        ...
        for (; elemIt != elemEndIt; ++elemIt) {
            ...
            for (; isIt != isEndIt; ++ isIt) {
                ...
                Ewoms::FaceDir::DirEnum faceDir;
                switch (insideFaceIdx) {
                case 0:
                case 1:
                    faceDir = Ewoms::FaceDir::XPlus;
                    break;

                case 2:
                case 3:
                    faceDir = Ewoms::FaceDir::YPlus;
                    break;

                case 4:
                case 5:
                    faceDir = Ewoms::FaceDir::ZPlus;
                    break;
...
private:
    ...
    void applyAllZMultipliers_(Scalar& trans,
                               ...
                               const Ewoms::TransMult& transMult,
                               ...
                               bool pinchTop)
    {
        if (insideFaceIdx > 3) { // top or or bottom
            ...
            // Last multiplier
            Scalar mult = transMult.getMultiplier(lastCartElemIdx , Ewoms::FaceDir::ZPlus);
            ...
            if ( !pinchTop )
            {
                ...
                {
                    mult = std::min(mult, transMult.getMultiplier(cartElemIdx, Ewoms::FaceDir::ZPlus));
                }
            }
    ...
    {
        const FieldPropsManager* fieldProps =
            (global) ? &(vanguard_.eclState().fieldProps()) :
            &(vanguard_.eclState().globalFieldProps());

        std::array<bool,3> is_tran {fieldProps->tran_active("TRANX"),
                                    fieldProps->tran_active("TRANY"),
                                    fieldProps->tran_active("TRANZ")};
    ...
    void computeFaceProperties(const Intersection& intersection,
                               const int insideElemIdx EWOMS_UNUSED,
                               const int insideFaceIdx EWOMS_UNUSED,
                               const int outsideElemIdx EWOMS_UNUSED,
                               const int outsideFaceIdx EWOMS_UNUSED,
                               DimVector& faceCenterInside,
                               DimVector& faceCenterOutside,
                               DimVector& faceAreaNormal,
     ...
     */
    std::tuple<std::vector<Ewoms::NNCdata>, std::vector<Ewoms::NNCdata> >
    ...
    void applyMultipliers_(Scalar& trans,
                           ...
                           const Ewoms::TransMult& transMult) const
    {
        ...
        switch (faceIdx) {
        case 0: // left
            trans *= transMult.getMultiplier(cartElemIdx, Ewoms::FaceDir::XMinus);
            break;
        case 1: // right
            trans *= transMult.getMultiplier(cartElemIdx, Ewoms::FaceDir::XPlus);
            break;

        case 2: // front
            trans *= transMult.getMultiplier(cartElemIdx, Ewoms::FaceDir::YMinus);
            break;
        case 3: // back
            trans *= transMult.getMultiplier(cartElemIdx, Ewoms::FaceDir::YPlus);
            break;

        case 4: // bottom
            trans *= transMult.getMultiplier(cartElemIdx, Ewoms::FaceDir::ZMinus);
            break;
        case 5: // top
            trans *= transMult.getMultiplier(cartElemIdx, Ewoms::FaceDir::ZPlus);
            break;
        }
    }
    ...
    Ewoms::ConditionalStorage<enableEnergy,
