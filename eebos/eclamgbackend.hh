...
/*!
 * \file
 * \copydoc Ewoms::Linear::EclAmgBackend
 */
#ifndef EWOMS_ECL_AMG_BACKEND_HH
#define EWOMS_ECL_AMG_BACKEND_HH

#include "eclcriterion.hh"

#include <ewoms/numerics/linear/parallelbasebackend.hh>
#include <ewoms/numerics/linear/bicgstabsolver.hh>
...
NEW_TYPE_TAG(EclAmgLinearSolver, INHERITS_FROM(ParallelBaseLinearSolver));

NEW_PROP_TAG(AmgCoarsenTarget);
NEW_PROP_TAG(LinearSolverMaxError);

//! The target number of DOFs per processor for the ecl algebraic
//! multi-grid solver
SET_INT_PROP(EclAmgLinearSolver, AmgCoarsenTarget, 5000);

SET_SCALAR_PROP(EclAmgLinearSolver, LinearSolverMaxError, 1e7);

SET_TYPE_PROP(EclAmgLinearSolver, LinearSolverBackend,
              Ewoms::Linear::EclAmgBackend<TypeTag>);
 ...
 * \ingroup Linear
 *
 * \brief Provides a linear solver backend using the ecl
 *        algebraic multi-grid (AMG) linear solver from DUNE-ISTL.
 */
template <class TypeTag>
class EclAmgBackend : public ParallelBaseBackend<TypeTag>
{
    ...
    // define the smoother used for the AMG
...
// typedef Dune::SeqILUn<IstlMatrix,Vector,Vector> SequentialSmoother;
    ...
    static_assert(std::is_same<SparseMatrixAdapter, IstlSparseMatrixAdapter<MatrixBlock> >::value,
                  "The EclAmgBackend linear solver backend requires the IstlSparseMatrixAdapter");

public:
    EclAmgBackend(const Simulator& simulator)
...
protected:
    ...
    std::shared_ptr<AMG> preparePreconditioner_()
    {
        ...
        // create the ecl scalar product and the ecl operator
#if HAVE_MPI
