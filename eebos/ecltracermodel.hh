...
public:
    ...
    void init()
    {
        const Ewoms::Deck& deck = simulator_.vanguard().deck();
...
protected:
                         ...
                         const int tracerIdx)
    {
        ...
        Scalar phaseVolume =
            Ewoms::decay<Scalar>(fs.saturation(tracerPhaseIdx_[tracerIdx]))
            *Ewoms::decay<Scalar>(fs.invB(tracerPhaseIdx_[tracerIdx]))
            *Ewoms::decay<Scalar>(intQuants.porosity());

        // avoid singular matrix if no water is present.
        phaseVolume = Ewoms::max(phaseVolume, 1e-10);
        ...
        if (std::is_same<LhsEval, Scalar>::value)
            ...
            tracerStorage =
                    phaseVolume
                    * Ewoms::variable<LhsEval>(tracerConcentration_[tracerIdx][globalDofIdx][0], 0);
    }
...

    {
        ...
        Scalar A = scvf.area();
        Scalar v = Ewoms::decay<Scalar>(extQuants.volumeFlux(tracerPhaseIdx));
        Scalar b = Ewoms::decay<Scalar>(fs.invB(tracerPhaseIdx_[tracerIdx]));
        Scalar c = tracerConcentration_[tracerIdx][globalUpIdx];

        if (inIdx == upIdx)
            tracerFlux = A*v*b*Ewoms::variable<TracerEvaluation>(c, 0);
        else
            tracerFlux = A*v*b*c;
    ...
    void linearize_(int tracerIdx)
    {
        ...
        for (const auto& well : wells) {
            ...
            for (auto& connection : well.getConnections()) {
                ...
                if (connection.state() == Ewoms::Connection::State::SHUT)
                    continue;
